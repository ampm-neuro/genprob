function [all_mtx, sort_idx_max, sort_max_pix, max_pix, all_mtx_unsort, min_pix] = image_mean_activity_timewarp(trl_mtx, trl_idx, frame_times, traces, neurons, trials, time_series_event_spacing, varargin)
...
%time_series_event_spacing = [0.2 1.1 1.0 2.0 8.5 2.0]; %old
%time_series_event_spacing = [0.23 1.03 0.98 2.0 5.31 2.0]; final median probe values
...
hm_cells = tw_activity_trial_hm_full_warp(trl_mtx, trl_idx,...
    frame_times, traces, neurons, trials,...
...
for irow = 1:size(all_mtx,1)
    max_pix(irow) = find(all_mtx(irow,:)==max(all_mtx(irow,:)),1); 
end
[sort_max_pix, sort_idx_max] = sort(max_pix);

% find min pixel
%
min_pix = nan(size(all_mtx,1),1); 
for irow = 1:size(all_mtx,1)
    min_pix(irow) = find(all_mtx(irow,:)==min(all_mtx(irow,:)), 1); 
end
%[sort_min_pix, sort_idx_min] = sort(min_pix);
...
figure; 
hold on 
plot(smooth(mean(all_mtx), 10), '-', 'linewidth', 2)
ylim([0 1])
