...
%
if isempty(last_category_points)
    y1_category_points = [max(0,(y2_height-y1_height)/2) cumsum(data1)];
    y1_category_points(end) = [];
    y1_category_points(numel(data1)/2+1 : end) = y1_category_points(numel(data1)/2+1 : end) + gap1_total;
else 
    y1_category_points=last_category_points;
end
...
%}
y2_0=y1_category_points(1)+(y1_height-y2_height)/2;
y2_category_points = [y2_0 y2_0+cumsum(data2)] ;
y2_category_points(end) = [];
y2_category_points(numel(data2)/2+1 : end) = y2_category_points(numel(data2)/2+1 : end) + gap2_total;
