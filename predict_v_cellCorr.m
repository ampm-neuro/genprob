...
for itgf = 1:length(training_group_folders)
    ...
    if contains(training_group_folder, 'mevar')
        [~,~,~,~,corr_mtx] = cell_turnover_timewarp_trials_multisubj('mevar', unq_subjs);
    elseif contains(training_group_folder, 'hivar')
        [~,~,~,~,corr_mtx] = cell_turnover_timewarp_trials_multisubj('hivar', unq_subjs);
    end
    ...
    for isubj = 1:size(corr_mtx,3)
        overlap_mtx_local = corr_mtx(:,:,isubj);
