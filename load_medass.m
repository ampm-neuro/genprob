function [medass_cell, trl_mtx, varargout] = load_medass(filepath)
...
%% load trial matrix
    ...
    %trl_mtx = [];
    trl_mtx = trial_mtx_new(medass_cell);
    if sum(isnan(trl_mtx(:,1)))>0
        error
    end
...
if any(contains(dir_hold, 'cnmfe_out.mat'))
