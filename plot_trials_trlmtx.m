function plot_trials_trlmtx(trl_mtx)
% plots major events of each trial as a row

% rich tones
load('unqfrq41.mat', 'unqfrq41');
rich_tones = unqfrq41(16:26);

% figure prep
%figure; 
hold on; set(gca,'TickLength',[0, 0]);
...
for itrl = 1:size(trl_mtx,1)
    ...
    if ~isnan(trl_mtx(itrl,11)) % rewarded trial
        ...
        if ismember(floor(trl_mtx(itrl,2)), rich_tones)
            plot([head_entry; trl_end], [1; 1]+(valid_trial_count-1), 'Color', [174 116 116]./255, 'linewidth', 1)
        else
            plot([head_entry; trl_end], [1; 1]+(valid_trial_count-1), 'Color', 0.5.*[1 1 1], 'linewidth', 1)
        end
        %}
        plot([head_entry; trl_end], [1; 1]+(valid_trial_count-1), 'Color', tcolors(ismember(unqfrq41, floor(trl_mtx(itrl,2))),:), 'linewidth', 1) % tone color
        plot([rwd_delivery;rwd_delivery], repmat([0.5;1.5], 1, length(rwd_delivery))+(valid_trial_count-1), 'Color', [22 117 15]./255, 'linewidth', 2.5) % rewarded line
    
    elseif trl_mtx(itrl,3) == 0 % reward unavailable
        %{
        if ismember(floor(trl_mtx(itrl,2)), rich_tones)
            %plot([head_entry; trl_end], [1; 1]+(valid_trial_count-1), 'Color', [166 28 28]./255, 'linewidth', 1.5)
            plot([head_entry; trl_end], [1; 1]+(valid_trial_count-1), 'Color', [174 116 116]./255, 'linewidth', 1.5)
        else
            plot([head_entry; trl_end], [1; 1]+(valid_trial_count-1), 'Color', 0.0.*[1 1 1], 'linewidth', 1.5)
        end
        %}
        plot([head_entry; trl_end], [1; 1]+(valid_trial_count-1), 'Color', tcolors(ismember(unqfrq41, floor(trl_mtx(itrl,2))),:), 'linewidth', 1.5) % tone color
...
% wait times plot
%figure; wait_times_plot_tonecolor(trl_mtx);
