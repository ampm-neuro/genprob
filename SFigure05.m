...
% combine
%all_rscore = [rscore_predict(:); rscore_unpredict(:)]; 
    all_rscore = rscore_unpredict(:); 
%all_days = [days_mtx_predict(:);days_mtx_unpredict(:)];
    all_days = days_mtx_unpredict(:);
all_rscore = all_rscore(~isnan(all_days));
all_days = all_days(~isnan(all_days));
    
% model time
ibins = [0:0.25:5 5.5:.5:30 31:1:40 42:2:60 65:5:100];
ibin_cell = cell(1,length(ibins)-1);
for ibin = 1:length(ibin_cell)
    ibin_cell{ibin} = all_rscore(all_days>ibins(ibin) & all_days<=ibins(ibin+1));
end
max_x = 100;
[~, coefEsts, modelFun] = ampm_normal_fit(all_days(~isnan(all_days) & all_days<max_x), all_rscore(~isnan(all_days) & all_days<max_x), [0 1 0 150]);

%plot data points, boxcar mean, and fit curve
figure; hold on; 
plot(all_days, all_rscore, 'o', 'color', 0.8.*[1 1 1])
...
%
    % predict
    figure; 
    subplot(1,2,1); imagesc(nanmean(rscore_predict,3)); 
    axis square; caxis([0 .6]); colorbar; title('original'); set(gca,'TickLength',[0, 0]);
    xticks(1:2:19); xlabel('Imaging session'); yticks(1:2:19); ylabel('Imaging session')
    rscore_predict = rscore_predict - modelFun(coefEsts, days_mtx_predict);
    subplot(1,2,2); imagesc(nanmean(rscore_predict,3)); 
    axis square; caxis([-.3 .3]); colorbar; title('Time corrected'); set(gca,'TickLength',[0, 0]);
    xticks(1:2:19); xlabel('Imaging session'); yticks(1:2:19); ylabel('Imaging session')
    sgtitle Predict
    ...
    % UNpredict
    figure; 
    subplot(1,2,1); imagesc(nanmean(rscore_unpredict,3)); 
    axis square; caxis([0 .6]); colorbar; title('original'); set(gca,'TickLength',[0, 0]);
    xticks(1:2:19); xlabel('Imaging session'); yticks(1:2:19); ylabel('Imaging session')
    rscore_unpredict = rscore_unpredict - modelFun(coefEsts, days_mtx_unpredict);
    subplot(1,2,2); imagesc(nanmean(rscore_unpredict,3)); 
    axis square; caxis([-.3 .3]); colorbar; title('Time corrected'); set(gca,'TickLength',[0, 0]);
    xticks(1:2:19); xlabel('Imaging session'); yticks(1:2:19); ylabel('Imaging session')
    sgtitle Unpredict
...
ylim([-.6 .6]); yticks(-1:.2:1); ylabel('Reactivation score')
...
save('rscores_timecorrect', 'rscore_predict_PreprobeProblem', 'rscore_unpredict_PreprobeProblem', 'rscore_predict_ProblemFinalprobe',...
...
%tbl = table(datamtx_adj_problem(:), first_day_discrimination(:), subj_num_mtx(:), session_num_mtx(:), 'VariableNames', {'PopOverlap', 'Discrim', 'Subject', 'Session'});
    ...
    plot2var(reshape(first_day_discrimination(group_mtx==0), length(predict_subjs), 6), rscore_predict_PreprobeProblem, ...
            ...
            axis([-5 5 -.6 .6]); xticks(-1:.2:1); yticks(-1:.2:1) 
...
%tbl = table(datamtx_last_problem(:), first_day_discrimination(:), subj_num_mtx(:), session_num_mtx(:), 'VariableNames', {'PopOverlap', 'Discrim', 'Subject', 'Session'});
    ...
    plot2var(rscore_predict_PreprobeProblem, rscore_predict_ProblemFinalprobe, ...
            ...
            axis([-.6 .6 -.6 .6]); xticks(-1:.2:1); yticks(-1:.2:1) 
