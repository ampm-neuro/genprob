...
for imtx = 1: length(cell_e)
    cell_e_xaxis{imtx} = nan(size(cell_e{imtx}));
        ...
        for idpt = 1:length(cell_e{imtx})
            ...
            base_jitter_multiplier = length(cell_e{imtx})*0.0006 + 0.15; %0.350;
            ...
            color_ct = color_ct+1;
            plot(cell_e_xaxis{imtx}(idpt), cell_e{imtx}(idpt), [line_type 'o'], 'color', colors(color_ct,:), 'markersize', 5)
        end
    %end
...
if connect == 1
    ...
    for subj = 1:size(cell_e_mtx,1)
        ...
        plot(cell_e_xaxis_mtx(subj,:), cell_e_mtx(subj,:), [line_type 'o'], 'markersize', 5, 'color', colors(color_ct,:))  
...
if ~exist('eb_color', 'var')
    if exist('xaxis', 'var') && ~isempty(xaxis)
        %bar(xaxis, means)
        errorbar(xaxis, means, std_es, ['k' line_type], 'linewidth', 3.0)
    else
        %bar(means)
        errorbar(means, std_es, ['k' line_type], 'linewidth', 3.0)
    end
else
    if exist('xaxis', 'var') && ~isempty(xaxis)
        %bar(xaxis, means)
        errorbar(xaxis, means, std_es, line_type, 'color', eb_color, 'linewidth', 3.0)
    else
        %bar(means)
        errorbar(means, std_es, line_type, 'color', eb_color, 'linewidth', 3.0)
    end
