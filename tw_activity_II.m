function act_mtx = tw_activity_II(trl_mtx, trl_idx, frame_times, traces, neurons, trials, align_event, tw_bounds)
...
for ic = 1:length(neurons)
    ...
    for itrl = 1:length(trials)
        ...
        if ~isempty(ae_time) && ~isnan(ae_time) % not all trials have rewards
            ...
            frametime_idx = frame_times>=tw_bounds_trl(1) & frame_times<=tw_bounds_trl(2) & trl_idx' == current_trial;
            frametimes_obs = frame_times(frametime_idx);
