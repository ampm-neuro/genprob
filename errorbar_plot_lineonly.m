function errorbar_plot_lineonly( cell_e , varargin)
%plots errorbar
...
if ~exist('eb_color', 'var')
    if exist('xaxis', 'var') && ~isempty(xaxis)
        %bar(xaxis, means)
        errorbar(xaxis, means, std_es, 'k-', 'linewidth', 1.85)
    else
        %bar(means)
        errorbar(means, std_es, 'k-', 'linewidth', 1.85)
    end
else
    if exist('xaxis', 'var') && ~isempty(xaxis)
        %bar(xaxis, means)
        errorbar(xaxis, means, std_es, '-', 'color', eb_color, 'linewidth', 1.85)
    else
        %bar(means)
        errorbar(means, std_es, '-', 'color', eb_color, 'linewidth', 1.85)
    end
