function [all_fixed_effects, all_beta_pvals, wait_times_all] = plot_allprobes_colorin(file_paths, color_in)
% plot all probes
% varargin is colors
...
%for inum = 1:max(pp_nums)
    ...
    if ~isempty(ppnum_fps)
        ...
        for ippnum = 1:size(ppnum_fps,1)
            ...
            %hold on; plot(frequencies_local, wait_times_local, '-', 'color', color_in)
        ...
        for ifrq = 1:length(unq_frq)
            mean_wait_times(ifrq) = nanmean(wait_times_all(:,frequencies_all(1,:)==unq_frq(ifrq)));
            se_wait_times(ifrq) = nanstd(wait_times_all(:,frequencies_all(1,:)==unq_frq(ifrq)))/sqrt(size(wait_times_all,1));
        end
        
        % plot

        %color_hold = rand(1,3);
        errorbar(unq_frq, mean_wait_times, se_wait_times, 'linewidth', 1.5, 'color', color_in)
