...
%
%
all_density_acc = all_cfos_cts_acc./all_area_acc;
%{
...
all_density_acc(setdiff(1:30, incl_subjs),:) = nan;
...
for igrp = 1:6
    ...
    % density
    %density_cell{igrp} = nanmean(all_density_acc(subject_condition==igrp, included_slices), 2); % drop bad slices
    density_cell{igrp} = mean(all_density_acc(subject_condition==igrp, included_slices), 2); % drop bad subjs
