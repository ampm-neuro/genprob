function [wait_prefs, rich_waits, poor_waits] = continuous_learning_singlesesh_suprise(session_path, wdw_size)
...
% window back and forwards
if length(wdw_size)==2
    wdw_back = wdw_size(1); wdw_fwd = wdw_size(2);
else
    wdw_back = floor(wdw_size/2); wdw_fwd = wdw_back;
end
...
for islide = 1:size(all_trl_mtx_hold,1)
    ...
    win_lo = islide-wdw_back; if win_lo<1; win_lo=1; end
    win_hi = islide+wdw_fwd; if win_hi>size(all_trl_mtx_hold,1); win_hi=size(all_trl_mtx_hold,1); end
   % win_lo = islide-floor(wdw_size); if win_lo<1; win_lo=1; end
    %win_hi = islide; if win_hi>size(all_trl_mtx_hold,1); win_hi=size(all_trl_mtx_hold,1); end
    
    islide_idx = ismember(1:size(all_trl_mtx_hold,1), win_lo:win_hi);

    if length(unique(session_number_hold(islide_idx)))>1
        wait_prefs(islide+wdw_fwd) = nan;
        continue
    end
