...
for itrl = 1:length(medass_cell{2})
        ...
        trial_start_he = max(medass_cell{12}(medass_cell{12} > trial_start_np & medass_cell{12} <= trial_end - fixed_delay_duration)); %head entry
        trial_start_fixed = trial_start_he + fixed_delay_duration;
        ...
        trial_end = medass_cell{4}(itrl);
        trial_end_he_offset = medass_cell{13}(medass_cell{13}>trial_start_fixed & medass_cell{13}<trial_end)';
